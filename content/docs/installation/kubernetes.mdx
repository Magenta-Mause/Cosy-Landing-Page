...
Type `2` and press **Enter** to select Kubernetes.
...
The domain that will be used for Kubernetes Ingress rules and CORS configuration. This should be the domain you plan to point at your cluster (e.g., `cosy.example.com`).
...
1. **Checks prerequisites** — Verifies `kubectl` and cluster connectivity (also handles `SUDO_USER` kubeconfig resolution)
2. **Creates the `cosy` namespace**
3. **Creates Kubernetes secrets** — PostgreSQL, InfluxDB, Loki, and admin credentials (5 secrets total)
...
6. **Waits for all pods to become ready** (300s timeout for deployments, 180s for statefulsets)
...
**Save the password.** Unlike the Docker installation, there is no `credentials.txt` file — the password is only shown once in the terminal output.
...
curl -fsSL https://raw.githubusercontent.com/magenta-mause/cosy/main/install_cosy.sh \
  | sudo bash -s -- kubernetes --default
```
