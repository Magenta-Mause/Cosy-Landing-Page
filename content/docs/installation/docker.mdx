...
Download and run the install script with the `docker` argument:

```bash
sudo bash -c "$(curl -fsSL https://raw.githubusercontent.com/Magenta-Mause/Cosy/v1.0.0/install_cosy.sh)" _ docker
```
...
The domain or IP address users will use to access COSY. This is used for CORS configuration. If you're running COSY on a server with a public domain, enter it here (e.g., `cosy.example.com`). For local testing, set this to `localhost`.
...
If you want to skip all prompts (e.g., for automated deployments), pass the `--default` flag to use defaults for all unset options:

```bash
sudo bash -c "$(curl -fsSL https://raw.githubusercontent.com/Magenta-Mause/Cosy/v1.0.0/install_cosy.sh)" _ docker --default
```
